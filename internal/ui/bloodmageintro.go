...
func BloodmageIntro() {
	bms := rl.LoadTexture("resources/textures/bms.jpg")
	...
	for !rl.WindowShouldClose() && active.Load() {
		...
		imgDestRect := rl.Rectangle{X: 50*vw - imgS/2, Y: 40 * vh, Width: imgS, Height: imgS}
		rl.DrawTexturePro(bms, imgSrcRect, imgDestRect, rl.Vector2{X: 0, Y: 0}, 0, rl.White)

		// draw title
		fontS := 5 * vmin
		ts := rl.MeasureTextEx(FontDefault, title, fontS, 1)
		rl.DrawTextEx(FontDefault, title, rl.Vector2{X: 50*vw - ts.X/2, Y: 40*vh - ts.Y*2}, fontS, 1, bloodRed)
		rl.EndDrawing()
	}
